...
2. 对于新手来说如果对于网络代理等基本的网络概念不熟悉的话就不能很好的理解这个工具的使用原理。
...
我们可以通过node去实现一个极简版本的普通代理，只对代理的HTTP请求转发到本来的目的地并返回目的地服务器的响应。代码如下（需要一些最基础的node有关网络服务相关的知识，这部分可以自己搜索，基本一看就懂）：
...
配置成功后我们就可以随便访问一个**http协议**的网站（目前只能是http协议的网站，具体原因之后讲解），通过对发送的请求和响应做出任何我们想要做的事情。比如最简单的打印请求的内容:
...
隧道代理的产生其实就是为了解决我们之前发现的普通代理没有办法解决对于加密传输内容代理的问题。为了更好的解释这个隧道代理是怎么解决这个问题，这需要我们先回忆一下HTTPS的工作原理从而搞清楚为什么普通代理不能代理HTTPS等加密请求。

#### HTTPS的工作原理

关于HTTPS的工作原理的主题的文章已经有了很多，这里介绍一篇讲得很清楚的文章。[HTTPS 详解一：附带最精美详尽的 HTTPS 原理图](https://segmentfault.com/a/1190000021494676)。如果有不清楚的同学可以先移步查看。

![image-20210628102152117](Whistle分享.assets/image-20210628102152117.png)
...
要理解为什么whistle要安装根证书，首先就要复习一下HTTPS的工作原理。
